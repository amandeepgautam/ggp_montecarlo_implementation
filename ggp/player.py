...
class RandomPlayer:
    ...
    def run_simulation(self, state, tSec):
        pass
...
class MonteCarloPlayer:
    ...
    def __init__(self, gd, sim, role, heuristic=None, maxDepth=-1):
        from ggp.explore import Explore
        self.gd = gd
        self.sim = sim
        self.role = role
        self.heuristic = heuristic
        self.maxDepth = maxDepth
        self.explore = Explore(gd, sim, role, self.shouldIStop, self.processResponse)

    def processResponse(self, er):
        """ Update the  response of the exploration.
        """
        self.move = er.bestMove
    ...
    def run_simulation(self, state, tSec):
        """
        Start simulation from the given state and for given number of seconds.
        """
        startTime = time.time()
        while(time.time() - startTime < tSec):
            self.explore.explore(state)
    ...
    def processReward(self, rewards):
        #Clear out transposition table at end of match
        print("Process reward: Kind of implemented.")
        #self.explore.tree = None
